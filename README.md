...
"plugin_instance": ...}}`), or to "previous" or "next" to step through the active playlist. A press shows
the screen straight away and moves the playlist position to it, so the cycle carries on from there. The listener (`src/buttons.py`) uses gpiod on GPIO 5, 6, 16 and 24 (A to D, top to bottom) and stays
